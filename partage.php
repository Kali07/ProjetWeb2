<?php 
     ...
     if (isset($_SESSION['id'])) { 
     	...
     	$req = $db->prepare('SELECT * FROM PARTAGE, PHOTO, COLLECTION WHERE Personne = ? AND PHOTO.Id_photo = PARTAGE.Photo AND COLLECTION.Id_collection = PHOTO.Collection');
                     ...
                     while ($image = $req->fetch())
                     {
                         ...
                         if ($it = opendir($dossier)) {
                             ...
                             if (false !== ($fichier = readdir($it))) {
                                    ?>
                                    <figure>
                                        <a href="photoPartage.php?nomCol=<?=$image['Nom']?> &amp nom_photo=<?=$image['Nom_img']?>"><img src="<?=$dossier.'/'.$image['Nom_img']?>" alt="icone de la photo"/></a>
